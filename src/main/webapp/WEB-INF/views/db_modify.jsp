...
function fn_regist_notice() {
    ...
    if(confirm("시간을 수정하시겠습니까?")) {

    	
    	document.board.action = "<c:url value='/db_update.do'/>";
...
function search(){
	...
	console.log(maxDay);
	window.location.href = "db_modify.do?userName=" + $("#user option:selected").val().toString() +"&month=" + $("#month option:selected").val().toString() + "&maxDay=" + maxDay;
}
...
function header() {
	return `
			...
			<a onclick="javascript:fn_regist_notice(); return fasle;">수정</a>
...
function body(date, inTime, outTime, totalTime){
	return `
			    ...
			    <td>\${date}일</td>
			    <td><input id="inTime" name="inTime" type="text" value="\${inTime}"  maxlength="6" > </td>
			    <td><input id="outTime" name="outTime" type="text" value="\${outTime}"  maxlength="6" > </td>
			    <td>\${totalTime}</td>
				</tr>
